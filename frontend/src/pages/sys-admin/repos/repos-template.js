...
class ReposTemplate extends Component {
  ...
  toggleNewFolderDialog = () => {
    this.setState({isNewFolderDialogShow: !this.state.isNewFolderDialogShow});
  }
  ...
  loadDirentList = (path) => {
    ...
    seafileAPI.sysAdminListSysRepoDirents(repoID, path).then(res => {
      let direntList = [];
  ...
  deleteDirent = (dirent) => {
    ...
    seafileAPI.sysAdminDeleteSysRepoDirent(this.props.repoID, path).then(res => {
  ...
  downloadItem = (dirent) => {
    let path = this.state.path + dirent.name;
    seafileAPI.sydAdminGetDownloadSysRepoItemURL(this.props.repoID, path).then(res => {
  ...
  uploadDirOrFile = () => {
    ...
    let { path } = this.state;
    seafileAPI.sydAdminGetUploadSysRepoItemURL(path).then(res => {
  ...
  render() {
    let { repoName, direntList, path, isNewFolderDialogShow } = this.state;
