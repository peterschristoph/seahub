...
class SysAdminShareToGroup extends React.Component {
  ...
  shareToGroup = () => {
    ...
    seafileAPI.sysAdminAddRepoSharedItemsPermission(repoID, 'group', this.state.permission, groups).then(res => {
      let errorMsg = [];
  ...
  deleteShareItem = (groupID) => {
    let repoID = this.props.repoID; 
    seafileAPI.sysAdminDeleteRepoSharedItemsPermission(repoID, 'group', groupID).then(() => {
      this.setState({
  ...
  onChangeUserPermission = (item, permission) => {
    ...
    seafileAPI.sysAdminUpdateRepoSharedItemsPermission(repoID, 'group', permission, groupID).then(() => {
