...
class SysAdminShareToUser extends React.Component {
  ...
  shareToUser = () => {
    ...
    seafileAPI.sysAdminAddRepoSharedItemsPermission(repoID, 'user' , this.state.permission, users).then(res => {
      let errorMsg = [];
  ...
  deleteShareItem = (useremail) => {
    let repoID = this.props.repoID;
    seafileAPI.sysAdminDeleteRepoSharedItemsPermission(repoID, 'user', useremail).then(res => {
      this.setState({
  ...
  onChangeUserPermission = (item, permission) => {
    ...
    seafileAPI.sysAdminUpdateRepoSharedItemsPermission(repoID, 'user', permission, userEmail).then(() => {
